        ...
        for b in range(k):
            for i in range(n):
                for j in range(k):
                    ...
                    # solver.add(And(sim_states[a][b], sim_states[i][j]) == R2[b][j])
                        solver.add(
                                Implies(sim_states[a][b],
                                Implies(Or((R1[a][i]==False), (R2[b][j]==False)),
                                 Or((sim_states[a][b]==False), (sim_states[i][j]==False)))))
                        # print(
                        #         Implies(sim_states[a][b],
                        #         Implies(Or(Not(R1[a][i]), Not(R2[b][j]),
                        #          And(Not(R1[a][i]),Not(R2[b][j]))),
                        #          Or(Not(sim_states[a][b]), Not(sim_states[i][j])))))
                    # print(Implies( Or(Not(R1[a][i]), Not(R2[b][j]), Not(Or(R1[a][i],R2[b][j]))),
                    #          Not(And(sim_states[a][b], sim_states[i][j]))))
...
# alpha_4_new_prophecy_aux = And([And(
#         [Implies(R1[i][j], And([Implies(sim_states[i][t],
#             And([Implies((R2[t][r] == False), (sim_states[j][r] == False)
#             ) for r in range(0,k) ])) for t in range(0,k)])
#             ) for j in range(0,n)]
#             ) for i in range(0,n)])
# alpha_4_new = And([And(
...
# print(alpha_4_new_prophecy_aux)
...
# solver.add(alpha_4_new_prophecy_aux)
...
solver.add(Not(sim_states[1][4]))
solver.add(Not(sim_states[2][1]))
solver.add(Not(sim_states[2][2]))
...
if (str(solver.check()) != "unsat"):
    ...
    def allProph1():
        for i in range(n):
            print("proph1("+str(i) + "): "+str(m.evaluate(x_prophs[i], model_completion=True)))
    def allProph2():
        for i in range(k):
            print("proph2("+str(i) + "): "+str(m.evaluate(y_prophs[i], model_completion=True)))
    # allSim()
    # allL1()
    # allL2()
    # allR2()
