...
def define_R_Transition():
    for a in range(n):
        for b in range(k):
            for i in range(n):
                for j in range(k):
                    solver.add(And(sim_states[a][b], sim_states[i][j], R2[b][j]) == Bool("R(sim"+str(a)+str(b)+"-sim"+str(i)+str(j)+")"))
                    # solver.add(Implies(Bool("R(sim"+str(a)+str(b)+"-sim"+str(i)+str(j)+")"), R2[b][j]))
...
# print(And([And([ sim_labels[i][j] == y_labels[j] for j in range(k)])
#         for i in range(n)]))
